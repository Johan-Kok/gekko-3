...
import (
	...
	genv "github.com/gottingen/gekko/env"
	"github.com/gottingen/gekko/gflag"
	"github.com/gottingen/gekko/mapping"
...
// mapping.DecoderConfig options
type DecoderConfigOption func(*mapping.DecoderConfig)

// DecodeHook returns a DecoderConfigOption which overrides the default
// DecoderConfig.DecodeHook value, the default is:
//
//  mapping.ComposeDecodeHookFunc(
//		mapping.StringToTimeDurationHookFunc(),
//		mapping.StringToSliceHookFunc(","),
//	)
func DecodeHook(hook mapping.DecodeHookFunc) DecoderConfigOption {
	return func(c *mapping.DecoderConfig) {
		c.DecodeHook = hook
	}
}

// Config is a prioritized configuration registry. It
...
// In the public interface for the config package so applications
// can use it in their testing as well.
func Reset() {
	l = New()
	SupportedExts = []string{"json", "toml", "yaml", "yml", "properties", "props", "prop", "hcl", "dotenv", "env"}
	SupportedRemoteProviders = []string{"consul"}
}
...
var SupportedRemoteProviders = []string{"consul"}

func OnConfigChange(run func(in fsnotify.Event)) { l.OnConfigChange(run) }
func (l *Config) OnConfigChange(run func(in fsnotify.Event)) {
	l.onConfigChange = run
}

func WatchConfig() { l.WatchConfig() }

func (l *Config) WatchConfig() {
...
func (l *Config) SetConfigFile(in string) {
	if in != "" {
		l.configFile = in
	}
}
...
func (l *Config) SetEnvPrefix(in string) {
	if in != "" {
		l.envPrefix = in
	}
}

func (l *Config) mergeWithEnvPrefix(in string) string {
	if l.envPrefix != "" {
...
// keys.
func (l *Config) getEnv(key string) (string, bool) {
...
func (l *Config) ConfigFileUsed() string { return l.configFile }
...
func (l *Config) AddConfigPath(in string) {
	if in != "" {
		absin := absPathify(in)
...
// provider is a string value, "consul" are currently supported.
// endpoint is the url. consul requires ip:port
...
func (l *Config) AddRemoteProvider(provider, endpoint, path string) error {
...
func (l *Config) AddSecureRemoteProvider(provider, endpoint, path, secretkeyring string) error {
	if !stringInSlice(provider, SupportedRemoteProviders) {
		return UnsupportedRemoteProviderError(provider)
	}
	if provider != "" && endpoint != "" {
...
func (l *Config) providerPathExists(p *defaultRemoteProvider) bool {
...
func (l *Config) searchMap(source map[string]interface{}, path []string) interface{} {
	if len(path) == 0 {
		return source
	}
...
//
// Note: This assumes that the path entries and map keys are lower cased.
func (l *Config) searchMapWithPathPrefixes(source map[string]interface{}, path []string) interface{} {
	if len(path) == 0 {
		return source
	}
...
func (l *Config) isPathShadowedInDeepMap(path []string, m map[string]interface{}) string {
...
// e.g., if "foo.bar" has a value in the given map, it “shadows”
//       "foo.bar.baz" in a lower-priority map
func (l *Config) isPathShadowedInFlatMap(path []string, mi interface{}) string {
	// unify input map
...
//       "foo.bar.baz" in a lower-priority map
func (l *Config) isPathShadowedInAutoEnv(path []string) string {
	var parentKey string
	for i := 1; i < len(path); i++ {
		parentKey = strings.Join(path[0:i], l.keyDelim)
...
func (l *Config) SetTypeByDefaultValue(enable bool) {
...
func Sub(key string) *Config { return l.Sub(key) }
func (l *Config) Sub(key string) *Config {
	subl := New()
	data := l.Get(key)
	if data == nil {
		return nil
...
func (l *Config) GetStringMap(key string) map[string]interface{} {
...
func (l *Config) GetStringMapString(key string) map[string]string {
	return cast.ToStringMapString(l.Get(key))
}

// GetStringMapStringSlice returns the value associated with the key as a map to a slice of strings.
func GetStringMapStringSlice(key string) map[string][]string { return l.GetStringMapStringSlice(key) }
func (l *Config) GetStringMapStringSlice(key string) map[string][]string {
...
func (l *Config) GetSizeInBytes(key string) uint {
...
func (l *Config) UnmarshalKey(key string, rawVal interface{}, opts ...DecoderConfigOption) error {
...
func (l *Config) Unmarshal(rawVal interface{}, opts ...DecoderConfigOption) error {
...
func defaultDecoderConfig(output interface{}, opts ...DecoderConfigOption) *mapping.DecoderConfig {
	c := &mapping.DecoderConfig{
		Metadata:         nil,
		Result:           output,
		WeaklyTypedInput: true,
		DecodeHook: mapping.ComposeDecodeHookFunc(
			mapping.StringToTimeDurationHookFunc(),
			mapping.StringToSliceHookFunc(","),
		),
	}
...
// A wrapper around mapping.Decode that mimics the WeakDecode functionality
func decode(input interface{}, config *mapping.DecoderConfig) error {
	decoder, err := mapping.NewDecoder(config)
	if err != nil {
		return err
	}
...
func (l *Config) UnmarshalExact(rawVal interface{}) error {
...
func (l *Config) BindPFlags(flags *gflag.FlagSet) error {
...
//	 Config.BindPFlag("port", serverCmd.Flags().Lookup("port"))
//
func BindPFlag(key string, flag *gflag.Flag) error { return l.BindPFlag(key, flag) }
func (l *Config) BindPFlag(key string, flag *gflag.Flag) error {
...
func (l *Config) BindFlagValues(flags FlagValueSet) (err error) {
...
//
//	 serverCmd.Flags().Int("port", 1138, "Port to run Application server on")
//	 Config.BindFlagValue("port", serverCmd.Flags().Lookup("port"))
//
func BindFlagValue(key string, flag FlagValue) error { return l.BindFlagValue(key, flag) }
func (l *Config) BindFlagValue(key string, flag FlagValue) error {
	if flag == nil {
...
func (l *Config) BindEnv(input ...string) error {
...
// Config will check in the following order:
// flag, env, config file, key/value store, default.
// Config will check to see if an alias exists first.
// Note: this assumes a lower-cased key given.
func (l *Config) find(lcaseKey string) interface{} {
...
func (l *Config) IsSet(key string) bool {
	lcaseKey := strings.ToLower(key)
	val := l.find(lcaseKey)
	return val != nil
}

// AutomaticEnv has Config check ENV variables for all.
// keys set in config, default & flags
func AutomaticEnv() { l.AutomaticEnv() }
func (l *Config) AutomaticEnv() {
	l.automaticEnvApplied = true
}

// SetEnvKeyReplacer sets the strings.Replacer on the config object
// Useful for mapping an environmental variable to a key that does
// not match it.
func SetEnvKeyReplacer(r *strings.Replacer) { l.SetEnvKeyReplacer(r) }
func (l *Config) SetEnvKeyReplacer(r *strings.Replacer) {
	l.envKeyReplacer = r
}
...
func (l *Config) RegisterAlias(alias string, key string) {
...
func (l *Config) realKey(key string) string {
...
func (l *Config) InConfig(key string) bool {
	// if the requested key is an alias, then return the proper key
	key = l.realKey(key)
...
func (l *Config) SetDefault(key string, value interface{}) {
...
func (l *Config) ReadInConfig() error {
...
func (l *Config) MergeInConfig() error {
...
func (l *Config) ReadConfig(in io.Reader) error {
	l.config = make(map[string]interface{})
...
func (l *Config) MergeConfig(in io.Reader) error {
...
func (l *Config) MergeConfigMap(cfg map[string]interface{}) error {
	if l.config == nil {
		l.config = make(map[string]interface{})
	}
...
func (l *Config) WriteConfig() error {
	filename, err := l.getConfigFile()
	if err != nil {
		return err
	}
...
func (l *Config) SafeWriteConfig() error {
	filename, err := l.getConfigFile()
	if err != nil {
		return err
	}
...
func (l *Config) unmarshalReader(in io.Reader, c map[string]interface{}) error {
...
func (l *Config) marshalWriter(f vfs.File, configType string) error {
	c := l.AllSettings()
	switch configType {
	case "json":
...
func (l *Config) ReadRemoteConfig() error {
	return l.getKeyValueConfig()
}

func WatchRemoteConfig() error { return l.WatchRemoteConfig() }
func (l *Config) WatchRemoteConfig() error {
	return l.watchKeyValueConfig()
}

func (l *Config) WatchRemoteConfigOnChannel() error {
	return l.watchKeyValueConfigOnChannel()
}

// Retrieve the first found remote configuration.
func (l *Config) getKeyValueConfig() error {
	if RemoteConfig == nil {
		return RemoteConfigError("Enable the remote features by doing a blank import of the config/remote package: '_ github.com/gottingen/gekko/config/remote'")
	}
...
func (l *Config) getRemoteConfig(provider RemoteProvider) (map[string]interface{}, error) {
...
// Retrieve the first found remote configuration.
func (l *Config) watchKeyValueConfigOnChannel() error {
	for _, rp := range l.remoteProviders {
...
// Retrieve the first found remote configuration.
func (l *Config) watchKeyValueConfig() error {
	for _, rp := range l.remoteProviders {
...
func (l *Config) watchRemoteConfig(provider RemoteProvider) (map[string]interface{}, error) {
...
func (l *Config) flattenAndMergeMap(shadow map[string]bool, m map[string]interface{}, prefix string) map[string]bool {
...
func (l *Config) mergeFlatMap(shadow map[string]bool, m map[string]interface{}) map[string]bool {
	// scan keys
outer:
	for k := range m {
...
func (l *Config) AllSettings() map[string]interface{} {
...
func (l *Config) SetFs(fs felix.Felix) {
	l.fs = fs
}
...
func (l *Config) SetConfigName(in string) {
	if in != "" {
		l.configName = in
		l.configFile = ""
	}
...
func (l *Config) SetConfigType(in string) {
	if in != "" {
		l.configType = in
	}
}
...
func (l *Config) SetConfigPermissions(perm os.FileMode) {
	l.configPermissions = perm.Perm()
}

func (l *Config) getConfigType() string {
	if l.configType != "" {
		return l.configType
	}
...
func (l *Config) getConfigFile() (string, error) {
	if l.configFile == "" {
...
func (l *Config) searchInPath(in string) (filename string) {
...
func (l *Config) findConfigFile() (string, error) {
