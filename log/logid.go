...
void creat_sign_f64(char* psrc, int slen, unsigned int* sign1, unsigned int * sign2)
{
    *sign1=0;
    *sign2=0;
    if(slen <= 4)
...
unsigned long calculate_log_id(char *network, char* client_addr, char* local_addr,
                          int pid, unsigned long sec, unsigned long usec)
{
    char connected_string[128] = {0};
    unsigned int connected_string_size = sizeof(connected_string);
	snprintf(connected_string, connected_string_size, "%s:%s/%s %ld %ld %d",
             network, client_addr, local_addr, sec, usec, pid);
    unsigned int high_int = 0;
    unsigned int low_int = 0;
    creat_sign_f64(connected_string, connected_string_size, &high_int, &low_int);
    unsigned long log_id = (unsigned long)(((unsigned long)(high_int) << 32) | (unsigned long)(low_int));
    return log_id;
}
*/
...
//     localAddr  - local kimny server Addr
//     pid        - kimny process pid
