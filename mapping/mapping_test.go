...
type EmbeddedSlice struct {
	SliceAlias `mapping:"slice_alias"`
	Vunique    string
}
...
type EmbeddedArray struct {
	ArrayAlias `mapping:"array_alias"`
	Vunique    string
}

type SquashOnNonStructType struct {
	InvalidSquashType int `mapping:",squash"`
}
...
type Tagged struct {
	Extra string `mapping:"bar,what,what"`
	Value string `mapping:"foo"`
}
